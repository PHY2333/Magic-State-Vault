...
based_on_commit: 6670995a68fd079bb84a586259168029af1cd2e3
END_BINDING
PRO_STATUS: COMPLETE
BEGIN_FILE::cf7cc9b76a404547903213b7b0b3d384
...
### $V\times W$ 也是向量空间，但双线性不等于线性

笛卡尔积 $V\times W$ 配上逐坐标运算后本身就是向量空间：
...
对两个因子而言，这个向量空间与外直和 $V\oplus W$ 典范同构，通常甚至使用同一个有序对模型；这与 $V,W$ 是否有限维无关。区别不在于 $V\times W$ “只是集合”，而在于我们要求映射满足什么结构。
...
要求固定 $w$ 时 $v\mapsto b(v,w)$ 线性，并且固定 $v$ 时 $w\mapsto b(v,w)$ 线性。它通常不是把乘积向量空间 $V\times W$ 当作源的一张普通线性映射。因为双线性给出
...
多出来的交叉项正是两个变量发生双线性交互的表现。张量积的作用，是为这种“分别线性、但通常不对有序对整体线性”的规则找到一个真正的线性定义域。
...
它对代表元无关。若把 $v$ 改成 $v+u$，其中 $u\in U$，则目标中的变化是
...
因为 $u\otimes w\in U\otimes_kW\subseteq N$。改变 $w$ 的代表元同理，变化落在 $V\otimes_kS$ 中。该规则还分别线性，所以张量积泛性质给出线性映射
...
反方向令 $q_V:V\to V/U$、$q_W:W\to W/S$ 为商映射。诱导映射
...
把 $U\otimes_kW$ 和 $V\otimes_kS$ 都送到零，因此 $N$ 包含在它的核中。它遂唯一下降为
...
$$
\Psi([v\otimes w])=[v]\otimes[w].
$$

在两侧的生成元上，

$$
(\Psi\circ\Phi)([v]\otimes[w])
=
[v]\otimes[w],
$$

$$
(\Phi\circ\Psi)([v\otimes w])
=
[v\otimes w].
$$

两侧都由这些元素张成，所以 $\Phi\Psi$ 与 $\Psi\Phi$ 都是恒等映射。于是
...
这个同构解释了为何公式 $[v]\otimes[w]\mapsto[v\otimes w]$ 的目标不能只是 $V\otimes_kW$：改变 $v$ 或 $w$ 的商类代表元时，$v\otimes w$ 会发生变化；只有在进一步商掉 $U\otimes_kW+V\otimes_kS$ 后，这个变化才变成零。

这里使用了系数是域这一点。对一般模，$U\otimes W\to V\otimes W$ 未必单射，因而不能无条件把 $U\otimes W$ 当作所在张量积中的子模；必须改写为诱导映射的像，或加入相应的平坦性条件。
...
的 $R$-平衡双线性映射具有相应泛性质。这里新增了共享环作用及其左右侧别，不能把普通 $k$-双线性泛性质无条件搬过来。用于链复形时，平衡关系先在链群层构造新的商复形，之后才在该复形中取同调；它与“商掉边界”不是同一个商。完整定义以及它与余不变量商（coinvariant quotient）的关系见 [[Balanced tensor product 与 coinvariant quotient]]。
...
1. 先确认局部规则在当前对象上确有定义。若输入是商类，要检查代表元无关；若目标是同调类，要先检查闭链性；若在 $R$ 上取张量，还要检查平衡关系。
...
END_FILE::cf7cc9b76a404547903213b7b0b3d384
END_RESPONSE::cf7cc9b76a404547903213b7b0b3d384
