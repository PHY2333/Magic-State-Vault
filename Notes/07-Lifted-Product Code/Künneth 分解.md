...
后续证明只使用“有限个纯张量项之和”这一结构，因此把上面所有 $\tau_{p,q}^{(j)}$ 合并成一个有限列表，并统一用 $i$ 编号。于是可以为每一项记录其所在分量的整数对 $(p_i,q_i)$，并把相应的 cycle 代表元重记为
...
这里不同的 $i$ 可以属于同一个 $(p,q)$ 分量。由于 $\kappa_n$ 已经良定义且是线性映射，接下来的条件不依赖所选展开。具体地，
