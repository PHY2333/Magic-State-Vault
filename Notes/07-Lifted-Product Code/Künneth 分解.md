...
即使 $\bigoplus_{p+q=n}H_p(C)\otimes_kH_q(D)$ 和 $H_n(C\otimes_kD)$ 维数相同，也只能推出它们抽象同构；Künneth 问题要证明的是，后面定义的比较映射 $\kappa_n$ 本身是同构映射，更是自然同构。
...
直和先唯一地确定 $\alpha$ 的各个 $(p,q)$ 分量；每个非零分量再写成有限个纯张量之和。把所有分量中的这些纯张量项合并成一个有限列表，为每一项记录其所在分量的整数对 $(p_i,q_i)$，并选择 cycles
...
这里的 $i$ 编号的是合并后的纯张量项，而不是直和分量；不同的 $i$ 可以属于同一个 $(p,q)$ 分量。直和分量本身唯一，但每个分量内的纯张量展开因张量积的双线性关系一般不唯一。由于 $\kappa_n$ 已经良定义且是线性映射，接下来的条件不依赖所选展开。具体地，
