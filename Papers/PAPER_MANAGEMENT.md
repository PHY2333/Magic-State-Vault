...
`Papers/SOURCES.md` 中的“主文献笔记”只登记 paper/book 层面的主笔记；翻译相关信息拆为“翻译计划”“当前全文译本”“局部摘译”三个字段。“翻译计划”只表示计划范围，后两个字段只保存当前文件链接；译文进度和核对状态不在来源登记中维护。由某篇来源支撑的全部主题概念笔记通过正文引用和反向链接查找，不在登记文件中重复维护完整清单。
...
本节的多版本规则只用于 `Papers/` 中的原始来源文件。译文仍采用第 5 节和 `Translations/TRANSLATION_GUIDE.md` 规定的单一当前译本模型，不为每个来源版本默认建立译文副本。
...
“翻译计划”不表示译文是否已经创建，也不记录“翻译中”“初译完成”“待核对”“已核对”等译文内部状态。当前全文译本和局部摘译位于 `Translations/` 时，从 `Papers/SOURCES.md` 链接分别使用 `../Translations/<文献ID>.full.zh-CN.md` 和 `../Translations/<文献ID>.excerpt.zh-CN.md`。

每个文献 ID 至多登记一个当前全文译本和一个当前局部摘译文件。译文必须在自身“译文信息”中绑定精确原文版本；来源更新为新版本时，先按 `Translations/TRANSLATION_GUIDE.md` 核对差异，再更新当前译文及其版本、覆盖范围和待核对项，不得静默把旧译文声明为适用于新版本。只有用户明确要求保留旧译本时才另行处理，本文件不建立默认的多版本译文命名或并存规则。
...
新增来源、登记新版本、改变阅读状态、关联主文献笔记，或更新翻译计划、当前全文译本及局部摘译链接时，只修改 `Papers/SOURCES.md`，不得在本文件保留来源记录副本。
...
- 规则变化只修改 `PAPER_MANAGEMENT.md`；来源登记内容变化只修改 `SOURCES.md`；关系变化只修改 `RELATIONS.md`。
...
- 分配或沿用了哪个稳定 ID；
- 登记了哪个版本或版次；
- 是否改变阅读状态、主文献笔记、翻译计划、当前全文译本或局部摘译链接；
